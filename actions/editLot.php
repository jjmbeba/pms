...
$lotId = $name = $capacity = $price = '';
$idErr = $nameErr = $capacityErr = $priceErr = $statementErr = $successMessage = '';
...
if (isset($_POST['submit'])) {
    ...
    if(empty($nameErr) && empty($priceErr) && empty($capacityErr)){
        $result = editLot($conn, $lotId, $name, $location, $capacity, $price);
        ...
        $statementErr = "$nameErr <br/> $priceErr <br/> $capacityErr <br/> $idErr";
        header("location:$redirect?error=$statementErr");
    }
}
